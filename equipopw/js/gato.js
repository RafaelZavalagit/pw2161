...
function iniciaGato()
{
	...
	if(typeof(Storage) != "undefined")  //Para preguntar si el navegador es compatible con "localStorage"
	{
		...
		{
			cuentajuegos=localStorage.webCuentaJuego;
			cuentajuegos++;
		}
		else
		{
		...
		document.getElementById("tituloJuego").innerHTML="Juego del Gato (#"+cuentajuegos+")";
	}
	else
...
function checar()
{
	...
	if(contador>=5)
	{
		if((document.getElementById('s11').value==document.getElementById('s12').value)&&(document.getElementById('s12').value==document.getElementById('s13').value)&&document.getElementById('s11').value!=""&&ganador==false)
		{
			...
			localStorage.webCuentaJuego=cuentajuegos; 
			location.reload();
			ganador=true;
			contador=0;
